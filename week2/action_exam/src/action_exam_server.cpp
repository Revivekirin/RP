...
class PrimenumberActionServer : public rclcpp::Node
{
public:
  ...
  : rclcpp::Node("action_exam_server")
  {
    ...
    server_ = rclcpp_action::create_server<Primenumber>(
      this,
      "action_exam_action",  // 액션 이름 (클라와 동일해야 함)
...
private:
      ...
      std::shared_ptr<const Primenumber::Goal> goal)
  {
    ...
    if (goal->target < 2) {
      RCLCPP_WARN(this->get_logger(), "target < 2: reject");
  ...
  void execute(const std::shared_ptr<GoalHandlePrimenumber> goal_handle)
  {
    ...
    auto result = std::make_shared<Primenumber::Result>();
    ...
    if (rclcpp::ok()) {
      ...
      RCLCPP_INFO(this->get_logger(), "Goal succeeded (found %zu primes).",
